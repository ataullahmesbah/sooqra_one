// src/components/Share/Shop/ShopProductCard/ShopProductCard.tsx
'use client';
...
// ── Interfaces (self-contained — no dependency on global Product type) ─────────
interface Price {
    currency: string;
    amount: number;
    exchangeRate?: number;
...
// ── Icon components ────────────────────────────────────────────────────────────
const CartIcon = ({ sm }: { sm?: boolean }) => (
    <svg className={sm ? 'w-3 h-3' : 'w-4 h-4'} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
    </svg>
);
const CheckIcon = ({ sm }: { sm?: boolean }) => (
    <svg className={sm ? 'w-3 h-3' : 'w-4 h-4'} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
    </svg>
);
const SpinIcon = ({ sm }: { sm?: boolean }) => (
    <svg className={`${sm ? 'h-3 w-3' : 'h-4 w-4'} animate-spin`} fill="none" viewBox="0 0 24 24">
...
// ── Main component ─────────────────────────────────────────────────────────────
const ShopProductCard: React.FC<{ product: ShopProduct }> = ({ product }) => {
    ...
    // ✅ FB ViewContent — once per mount, using ref not setState
    useEffect(() => {
        if (product && mainPrice && !hasTrackedView.current) {
            hasTrackedView.current = true;
            trackViewContent(product as any, mainPrice.amount);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [product._id]);

    // ✅ Inject toast CSS once globally (deduplicated by id)
    useEffect(() => {
        if (document.getElementById('shop-card-toast-css')) return;
        const s = document.createElement('style');
        s.id = 'shop-card-toast-css';
        s.textContent = `
            ...
            .shop-toast .tc { display: flex; align-items: center; gap: 10px; }
            .shop-toast .ti { width: 18px; height: 18px; flex-shrink: 0; color: #374151; }
            .shop-toast.success .ti { color: #10b981; }
            .shop-toast.error   .ti { color: #ef4444; }
            .shop-toast .tm { flex: 1; font-size: 13px; line-height: 1.4; }
            .shop-toast .tb { background: none; border: none; color: #9ca3af; cursor: pointer; padding: 3px; border-radius: 3px; }
        `;
        document.head.appendChild(s);
    }, []);

    // ✅ Fetch variants — no setState in effect body directly
    useEffect(() => {
    ...
    const showToast = useCallback((message: string, type: 'success' | 'error') => {
        ...
        const icon = type === 'success'
            ? `<svg class="ti" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>`
            : `<svg class="ti" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>`;

        el.innerHTML = `<div class="tc">${icon}<span class="tm">${message}</span><button class="tb" onclick="this.closest('.shop-toast').remove()">✕</button></div>`;
    ...
    // ── Availability ──────────────────────────────────────────────────────────
    const getActualAvailability = useCallback(() => {
    ...
    const cartBtnStyle = (() => {
        if (product.productType === 'Affiliate' || actualAvailability !== 'InStock')
            return 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-100';
            ...
            if (existing) {
                ...
                const priceInBDT = variant
                    ...
                    : (() => {
                        ...
                        return p.currency === 'BDT' ? p.amount : p.amount * 1;
                    })();
                cart.push({
                    ...
                    size: size || undefined,
                    variantId: variant?._id, variantName: variant?.name, variantWeight: variant?.weight,
                });
                showToast(`Added: ${product.title}`, 'success');
            }

            localStorage.setItem('cart', JSON.stringify(cart));
            trackAddToCart(product as any, 1, mainPrice?.amount || 0);
    ...
    }, [product, mainPrice, showToast, trackAddToCart]);

    const handleAddToCart = useCallback(() => {
        if (actualAvailability === 'Affiliate' || actualAvailability !== 'InStock') {
        ...
        if (product.sizeRequirement === 'Mandatory' && product.sizes) {
            if (!product.sizes.some(s => s.quantity > 0)) { showToast('No sizes available', 'error'); return; }
            setModalSelectedSize(null); setModalShowSizeError(false); setIsSizeModalOpen(true); return;
        }
        if ((product.quantity ?? 0) <= 0) { showToast('Out of stock', 'error'); return; }
        addToCart(null);
    }, [actualAvailability, product, variants, addToCart, showToast]);
    ...
    return (
        <>
            {/* ── Grid card ─────────────────────────────────────────────────── */}
            <div className="group bg-white rounded-xl border border-gray-100 hover:border-gray-300 hover:shadow-lg transition-all duration-300 overflow-hidden h-full flex flex-col">
                {/* Image */}
                <Link href={productUrl}>
                    <div className="relative aspect-square overflow-hidden bg-gray-100">
                        ...
                        <div className="absolute top-2 left-2 flex flex-col gap-1">
                            <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-sm ${product.productType === 'Affiliate' ? 'bg-purple-500 text-white' :
                            ...
                            {hasDiscount && (
                                <span className="text-[9px] font-bold px-1.5 py-0.5 bg-red-600 text-white rounded-sm">
                                    -{discountPercentage}%
                                </span>
                            )}
                        </div>
                ...
                <div className="flex-1 p-2 sm:p-3 flex flex-col">
                    <Link href={productUrl} className="flex-1 mb-1.5">
                            ...
                            <div className="flex flex-col gap-0.5">
                                ...
                                {hasDiscount && (
                                    <span className="text-[10px] text-gray-400 line-through">
                                        {mainPrice.currency === 'BDT' ? '৳' : '$'}{(mainPrice.amount * 1.15).toLocaleString()}
                                    </span>
                                )}
                            </div>
                        )}
                    ...
                    <div className="mt-auto">
                        ...
                        <div className="flex flex-col gap-1 sm:hidden">
                            <Link href={productUrl}
                                className="w-full py-1.5 bg-gray-800 text-white font-medium text-[10px] rounded hover:bg-gray-700 transition text-center">
                                Buy Now
                            </Link>
                            {product.productType === 'Affiliate' ? (
                                <button disabled className="w-full py-1.5 bg-gray-100 text-gray-400 text-[10px] rounded cursor-not-allowed">Affiliate</button>
                            ) : (
                                <button onClick={handleAddToCart} disabled={actualAvailability !== 'InStock' || isAddingToCart}
                                    className={`w-full py-1.5 text-[10px] rounded transition flex items-center justify-center gap-1 ${cartBtnStyle}`}>
                                    {isAddingToCart ? <><SpinIcon sm /><span>Adding…</span></> :
                                        showSuccess ? <><CheckIcon sm /><span>Added!</span></> :
                                            <><CartIcon sm /><span>Add to Cart</span></>}
                                </button>
                            )}
                        </div>
                        ...
                        <div className="hidden sm:flex gap-1.5">
                            <Link href={productUrl}
                                className="flex-1 py-2 bg-gray-800 text-white font-semibold text-xs rounded-lg hover:bg-gray-700 transition flex items-center justify-center whitespace-nowrap">
                                Buy Now
                            </Link>
                            {product.productType === 'Affiliate' ? (
                                ...
                                <button onClick={handleAddToCart} disabled={actualAvailability !== 'InStock' || isAddingToCart}
                                    className={`flex-1 py-2 text-xs rounded-lg transition flex items-center justify-center gap-1 whitespace-nowrap ${cartBtnStyle}`}>
                                    {isAddingToCart ? <><SpinIcon /><span>Adding…</span></> :
                                        showSuccess ? <><CheckIcon /><span>Added!</span></> :
            ...
            {/* ── Variant Modal (bottom sheet mobile) ───────────────────────── */}
            {isVariantModalOpen && (
                    ...
                    <div className="bg-white w-full sm:max-w-xl rounded-t-2xl sm:rounded-xl shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="px-4 sm:px-5 pt-3 pb-5">
                            ...
                            <button onClick={handleVariantAdd} disabled={isAddingToCart || !selectedVariant}
                                ...
                                {isAddingToCart ? <><SpinIcon /><span>Adding…</span></> : <><CartIcon /><span>Add to Cart</span>{selectedVariant && <span className="text-[10px] opacity-75">(৳{selectedVariant.price.toLocaleString()})</span>}</>}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {/* ── Size Modal (bottom sheet mobile) ──────────────────────────── */}
            {isSizeModalOpen && (
                    ...
                    <div className="bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-xl shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="px-4 sm:px-5 pt-3 pb-5">
                            ...
                            <button onClick={handleSizeAdd} disabled={isAddingToCart || !modalSelectedSize}
                                ...
                                {isAddingToCart ? <><SpinIcon /><span>Adding…</span></> : <><CartIcon /><span>Add to Cart</span>{modalSelectedSize && <span className="text-[10px] opacity-75">({modalSelectedSize})</span>}</>}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <CartSlider isOpen={isCartOpen} setIsOpen={setIsCartOpen} conversionRates={{ USD: 123, EUR: 135, BDT: 1 }} />
        </>
    );
};
