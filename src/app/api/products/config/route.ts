...
export async function POST(request) {
    try {
        ...
        const { code, discountAmount, minCartTotal, expiresAt } = await request.tson();
        ...
        if (!code || discountAmount == null || minCartTotal == null || !expiresAt) {
            ...
            return NextResponse.tson({ error: 'All fields are required' }, { status: 400 });
        }
        if (discountAmount < 0) {
            console.log('Validation failed: Negative discount amount');
            return NextResponse.tson({ error: 'Discount amount cannot be negative' }, { status: 400 });
        }
        if (minCartTotal < 0) {
            console.log('Validation failed: Negative minCartTotal');
            return NextResponse.tson({ error: 'Minimum cart total cannot be negative' }, { status: 400 });
        }
        const expiresAtDate = new Date(expiresAt);
        if (isNaN(expiresAtDate.getTime()) || expiresAtDate < new Date()) {
            console.log('Validation failed: Invalid or past expiry date');
            return NextResponse.tson({ error: 'Invalid or past expiry date' }, { status: 400 });
        }
        ...
        return NextResponse.tson({ message: 'Global coupon updated' }, { status: 200 });
    } catch (error) {
        console.error('Error in POST /api/products/config:', error.message, error.stack);
        return NextResponse.tson({ error: `Failed to update global coupon: ${error.message}` }, { status: 500 });
    }
}